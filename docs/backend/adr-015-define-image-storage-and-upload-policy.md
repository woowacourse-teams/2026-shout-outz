...
- `V20260831162619__init_schema.sql`에 프로젝트·포스트·사용자 프로필 스키마가 정의되어 있으므로, 미디어 관련 테이블은 `V20260914000000__create_media_schema.sql` 하나에서 생성한다.
...
- 미디어 메타데이터, 업로더 정보, 프로젝트·프로필 직접 참조, 포스트 매핑과 관련 인덱스·제약은 `V20260914000000__create_media_schema.sql` 하나에서 최종 형태로 생성한다. 사용하지 않는 `PROJECT_MEDIA` 용도는 처음부터 포함하지 않는다.

### 11. 포스트 미디어 매핑

- 프로젝트 썸네일은 `projects.thumbnail_media_id`, 사용자 프로필 이미지는 `user_profiles.avatar_image_id`가 `media_metadata.id`를 직접 참조하도록 관리한다. 프로젝트 본문 인라인 이미지는 `projects.description_md`에 `media://{mediaId}` 내부 참조를 포함한 Markdown으로 저장한다.
- `projects.thumbnail_media_id`와 `user_profiles.avatar_image_id`는 미디어 메타데이터 삭제 시 참조를 `NULL`로 정리하도록 `ON DELETE SET NULL`을 적용한다.
...
- 초기 모델에서 기존 프로젝트 썸네일·사용자 프로필 URL 컬럼은 `V20260831162619__init_schema.sql`에 남겨 두되, `V20260914000000__create_media_schema.sql`에서 제거한다. 최종 스키마에서는 `thumbnail_media_id`, `avatar_image_id`로 `media_metadata.id`를 참조한다. Markdown 본문은 `media://{mediaId}` 참조를 사용하며, 본문 저장·렌더링 통합은 별도 단계로 진행한다.
- `post_media`와 조회용 인덱스는 `V20260914000000__create_media_schema.sql`에서 `media_metadata`와 함께 생성한다.
- 사용자 프로필 이미지 참조 컬럼·외래키·인덱스는 `V20260914000000__create_media_schema.sql`에서 `avatar_image_id` 기준으로 생성한다.
...
- 삭제되지 않은 포스트에 `post_media`로 연결된 `POST_CONTENT` 미디어, 승인되고 삭제되지 않은 프로젝트의 `thumbnail_media_id`로 연결된 썸네일, 삭제되지 않은 사용자의 `avatar_image_id`로 연결된 프로필 이미지는 비로그인 조회를 허용한다. 프로젝트 본문 인라인 이미지는 본문 안의 `media://{mediaId}`만으로 리소스 연결을 DB에서 검증할 수 없으므로, 별도 본문 조회 통합 전까지 업로더 본인만 조회할 수 있다.
