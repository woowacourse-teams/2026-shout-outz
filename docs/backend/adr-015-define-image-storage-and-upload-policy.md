...
이번 결정은 이후 S3 리소스 설정, 미디어 테이블, 업로드·조회 API가 따라야 할 공통 기준을 정의한다. 구체적인 AWS 리소스 생성과 DB 스키마는 후속 단계에서 구현한다.
...
- 이번 ADR은 AWS 버킷·IAM·CORS의 구체적인 설정과 DB 테이블 구조를 확정하지 않는다. 각각 후속 구현 단계에서 반영한다.
