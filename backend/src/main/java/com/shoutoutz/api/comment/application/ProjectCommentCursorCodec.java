...
/**
 * 프로젝트 댓글 목록의 정렬 기준과 마지막 루트 댓글 위치를 opaque cursor로 변환한다.
 *
 * cursor는 현재 ID와 CREATED_AT를 조합하여 사용한다.
 */
public final class ProjectCommentCursorCodec {
    ...
    private ProjectCommentCursorCodec() {
    }

    public static ProjectCommentCursor decode(String encodedCursor) {
        ...
        try {
            ...
            ProjectCommentSort sort = ProjectCommentSort.valueOf(extract(payload, SORT_PATTERN));
            if (id <= 0) {
                throw new InvalidInputException(CommentErrorCode.INVALID_COMMENT_CURSOR);
            }
            return new ProjectCommentCursor(createdAt, id, sort);
        } catch (RuntimeException exception) {
            throw new InvalidInputException(CommentErrorCode.INVALID_COMMENT_CURSOR);
        }
    }

    public static String encode(ProjectCommentCursor cursor) {
