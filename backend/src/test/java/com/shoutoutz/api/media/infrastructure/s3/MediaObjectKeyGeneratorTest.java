...
class MediaObjectKeyGeneratorTest {
    ...
    void 미디어_용도와_UUID로_객체_키를_생성한다() {
        String key = keyGenerator.generate(MediaPurpose.FEED_CONTENT);

        assertThat(key).matches("media/feed-content/[0-9a-f-]{36}");
    }

    @Test
    void 원본_키에서_표시용과_썸네일_키를_결정적으로_생성한다() {
        String sourceKey = "media/feed-content/object-id";
