...
export type RequestOptions = Omit<Options, 'method'> & { method: HttpMethod };
...
export const httpClient = async <T>(url: string, options: RequestOptions): Promise<T | null> => {
  ...
  if (!isApiSuccessBody(body)) {
    throw new Error(`응답이 공통 규격을 따르지 않습니다: ${options.method.toUpperCase()} ${url}`);
  }
