...
import { createRemoteApp, fetchRemoteState, recordRemotePlay, setRemoteBookmark, toggleRemoteLike, updateRemoteApp, upsertRemoteProfile } from './utils/supabaseData'
...
function App() {
  ...
  const toggleBookmark = useCallback((id: string) => {
    ...
    void setRemoteBookmark(authUser.id, id, !wasBookmarked).catch(() => {
      ...
      setDataError('저장 상태를 반영하지 못했습니다. 잠시 후 다시 시도해주세요.')
    })
  ...
  const submitPage = authLoading ? <AuthLoadingPage /> : authUser && currentMaker ? dataStatusPage ?? <SubmitPage maker={currentMaker} onAddApp={addApp} /> : loginPage
