...
export function MakerPage({ apps, deletedApps, profile, currentMaker, isOwnProfile = false, bookmarkedIds, onSaveProfile, onToggleBookmark, onRestoreApp }: MakerPageProps) {
  ...
  if (isCurrentMakerPage && currentMaker && (isEditing || !profile)) {
    return <ProfileSetup profile={profile} avatarUrl={currentMaker.avatarUrl} onCancel={profile ? () => setIsEditing(false) : undefined} onSave={(nextProfile) => { onSaveProfile(nextProfile); setIsEditing(false); setSaveMessage(profile ? '수정이 완료되었습니다.' : '프로필이 등록되었습니다.') }} />
  }
  ...
  return (
    <>
      ...
      {saveMessage ? <div className="profile-save-message" role="status" aria-live="polite">{saveMessage}</div> : null}
    </>
  )
}
...
const MAX_PROFILE_BIO_LINES = 4

function normalizeProfileBio(value: string) {
  return value
    .replace(/\r\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .split('\n')
    .slice(0, MAX_PROFILE_BIO_LINES)
    .join('\n')
}
...
export function ProfileSetup({ profile, avatarUrl, onCancel, onSave }: { profile: Maker | null; avatarUrl?: string | null; onCancel?: () => void; onSave: (maker: Maker) => void }) {
  ...
  const [bio, setBio] = useState(() => normalizeProfileBio(profile?.bio ?? ''))
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (normalizedBio !== bio) setBio(normalizedBio)
    if (!name.trim()) {
      setError('이름을 입력해주세요.')
      return
    }
    ...
    if (!normalizedBio.trim()) {
      setError('짧은 소개를 입력해주세요.')
      return
    }
    onSave({ ...previewMaker, name: name.trim(), role: role.trim(), bio: normalizedBio.trim() })
  }
      ...
      <div className="content-container">
        ...
        <div className="profile-setup__card">
          ...
          <form className="profile-form" onSubmit={handleSubmit} noValidate>
            ...
            <div className="form-field"><div className="form-field__label"><label htmlFor="profile-bio">짧은 소개<span aria-hidden="true">*</span></label><span>최대 100자 · 최대 4줄</span></div><textarea id="profile-bio" value={bio} maxLength={100} onChange={(event) => { setBio(normalizeProfileBio(event.target.value)); setError('') }} placeholder="간단한 자기소개를 작성해주세요." rows={4} /></div>
